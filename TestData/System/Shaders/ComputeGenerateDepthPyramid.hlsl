...
void ComputeMain(in uint2 threadId: SV_DispatchThreadId)
{
    ...
    float4 depthValues = depthBuffer.Gather(TextureSampler, threadId / float2(parameters.DepthPyramidBufferWidth, parameters.DepthPyramidBufferHeight));

    float minDepthValue = min(min(min(depthValues.x, depthValues.y), depthValues.z), depthValues.w);
